...
Every word the keyboard has learned from your typing, plus any you add by hand, sorted with the highest weight first. **Sort by** above the list changes that to **Alphabetical** (words that start with a capital letter first) or **Date added**, and the button under it reverses the direction. The screen remembers the order. Each row reads "You added this word" once its weight reaches 200, and "Seen N times" below that. Press a row to open an edit dialog where you can respell the word or change that weight, anywhere from 1 to 1,000,000. New words the keyboard is still counting are not on this screen: they only appear once they have been typed enough times to be saved. See [Personal dictionary](/smart/personal-dictionary/#words-wait-their-turn).
