...
- **Fonts named by a theme.** A `.flex` names a font family rather than carrying the file, and a font here is a separate add-on, so there is nothing to install from the file itself. Install the font yourself and pick it in the theme editor. A theme that sets `font-family: inherit`, which is what both of the big theme packs do on every rule, is asking for no font change at all, so nothing is reported for it.
