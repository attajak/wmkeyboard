...
- The key grid itself: its letters layer and its number and symbol layers
...
- **Extra layers.** Some keyboards keep more pages than letters, numbers and symbols: a right-Alt layer, or one page per consonant holding its vowel forms. WM Keyboard doesn't show those yet, so the keys on them are out of reach.
- **Keys that exist for a rule to act on.** A key with no character of its own types the label printed on it rather than going through the rules, and so do flicks and long-press alternates.
