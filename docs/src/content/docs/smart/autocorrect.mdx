...
:::note
Missing apostrophes ("dont" &rarr; "don't") are fixed by a separate, fixed lookup of common contractions. That lookup runs *before* autocorrect even looks at the word, so it's a known-contraction fix rather than a confidence-scored guess, and it wins whenever both could apply. The contraction list is English. French gets the same fix for its elisions, read from the French word list instead of a table: "cest" becomes "c'est", "jai" becomes "j'ai", "lami" becomes "l'ami" and "quil" becomes "qu'il", and the apostrophised form is offered on the strip as you type. The keyboard knows which prefixes elide and which short words each one takes, so "jet" is never split into "j'et" and "don" never into "d'on". A real word that happens to split, such as "lune" or "tas", stays as typed; "l'une" and "t'as" sit behind it on the strip instead. It has its own toggle, "Fix missing apostrophes," in the same settings group, and no other language is read this way. Offensive-word filtering and the suggestion blacklist also keep those words out of the plain suggestion strip, not only out of autocorrect. See [Suggestion blacklist](/smart/blacklist/) for the full mechanism.
:::
