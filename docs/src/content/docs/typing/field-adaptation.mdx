...
- **URL fields** (`FieldKind.URI`): the comma key becomes <KeyCap>/</KeyCap> (long-press `?`, `#`, `&`, `=`), and the period key's long-press gains `.com`, `.org`, `.net`, `www.`, `https://`, and the `,` the comma key gave up, since a browser's address bar doubles as its search box.
