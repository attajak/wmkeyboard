...
- **Bold label** and **Label size**: how the label is drawn. Bold is a three-way choice — Automatic follows the board, On and Off answer for this key. Label size replaces the automatic rule rather than scaling it, which is how you get a `?123` drawn at full letter size. A size the *layout* gave the key in the [layout editor](/layouts/editor/) wins over the theme's, since the layout is the more specific answer.
