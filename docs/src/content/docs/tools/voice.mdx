...
The keyboard's own text boxes, such as the AI chat and its Custom prompt, the emoji, GIF, dictionary and clipboard searches, Find and replace, the clip editor and a plugin's text box, carry a small mic at the left of their suggestion strip. Tap it and speak: the phrase goes into that box at its caret, in place of any selected text, and never into the app behind the keyboard. The Voice tool does the same while one of these boxes has the keys.
