...
just `full`. A record from an F-Droid install is refused with a note instead:
F-Droid compiles the app on their own machines, so its mapping never existed
here. An older record that names no language build is retraced against both
candidates and the comment says which one resolved more frames.
...
GitHub release, but F-Droid signs with its own key, so moving to it from an
F-Droid install means uninstalling first.
