...
6. **Date the settings links.** In `docs/`, run
   `node scripts/extract-settings-since.mjs` and commit
   `src/data/settings-since.json`. It reads the tags to find which release
   first has each settings screen and row, which the `/open/` page, the link
   builder and the settings chips show as "Added in version …". Until it runs,
   what the new tag shipped still reads **Not released yet** on the site.
